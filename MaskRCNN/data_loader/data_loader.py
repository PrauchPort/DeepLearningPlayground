...
def build_detection_targets(rpn_rois, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    assert gt_class_ids.dtype == np.int32, "Expected int but got {}".format(gt.class_ids.dtype)
    ...
    instance_ids = np.where(gt_class_ids)[0]
    assert instance_ids.shape[0] > 0, "Instance must contain instances."
    ...
    for i in pos_ids:
        ...
        if config.USE_MINI_MASK:
            ...
            # Resize mini mask the size of GT box
...
def build_rpn_targets(image_shape, anchors, gt_class_ids, gt_boxes, config):
    ...
    # Handle COCO crowds
    # A crowd in COCO is a bounding box around several instances. Exclude
    # them for training. A crowd box is given a negative class ID.
    ...
    if crowd_ix.shape[0] > 0:
        ...
        crowd_iou = np.amax(crowd_overlaps, axis=1)
    ...
    #
    # 1. Set negative anchors first. They get overwritten below if a GT box
    # is matched to them. Skip boxes in crowd areas.

    anchor_iou_argmax = np.argmax(overlaps, axis=1)
    anchor_iou_max = overlaps[np.arange(overlaps.shape[0]), anchor_iou_argmax]
    rpn_match[(anchor_ioud_max < 0.3) * (no_crowd_bool)] = -1
    # 2. Set an anchor for each GT box (regardless of IoU value).
    # If multiple anchors have the same IoU match all of them
    gt_ioud_argmax = np.argwhere(overlaps == np.max(overlaps, axis=0))[:, 0]
    rpn_match[gt_iou_argmax] = 1
    # 3. Set anchors with high overlap as positive.
    rpn_match[anchor_iou_max >= 0.7] = 1

    # Subsample to balance positive and negative anchors
    # Don't let positives be more than hals the anchors
    ...
    for i, a in zip(ids, anchors[ids]):
        ...
        # Normalize
        rpn_box[ix] /= config.RPN_BBOX_STD_DEV
        ix += 1
...
def generate_random_rois(image_shape, count, gt_class_ids, gt_boxes):
    ...
    while True:
        ...
        threshold = 1
        y1y2 = y1y2[np.abs(y1y2[:, 0] - y1y2[:, 1]) >=
                    theshold][:remaining_count]
        x1x2 = x1x2[np.abs(x1x2[:, 0] - x1x2[:, 1]) >= 
                    threshold][:remaining_count]
        if y1y2.shape[0] == remaining_count and x1x2.shape[0] == remaining_count:
            break
    
    # Sort an axis 1 to ensure x1 <= x2 and y1 <= y2 and then reshape
    # into x1, y1, x2, y2 order
    x1, x2 = np.split(np.sort(x1x2, axis=1), 2, axis=1)
    y1, y2 = np.split(np.sort(y1y2, axis=1), 2, axis=1)
                ...
                if b >= batch_size:
                    ...
                    if random_rois:
                        ...
                        if detection_targets:
                            ...
                            batch_mrcn_class_ids = np.expand_dims(
