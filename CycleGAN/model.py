...
class Encoder(keras.Model):
    ...
    def __init__(self):
        ...
        self.conv1 = keras.layers.Conv2D(
            32, kernel_size=7, strides=1, kernel_initializer=tf.random_normal_initializer(
                stddev=0.02))

        self.conv2 = keras.layers.Conv2D(
            64, kernel_size=3, strides=2, kernel_initializer=tf.random_normal_initializer(
                stddev=0.02))

        self.conv3 = keras.layers.Conv2D(
            128, kernel_size=3, strides=2, kernel_initializer=tf.random_normal_initializer(
                stddev=0.02))
...
class Residual(keras.Model):
    ...
    def __init__(self):
        ...
        self.conv1 = keras.layers.Conv2D(
            128, kernel_size=3, strides=1, kernel_initializer=tf.random_normal_initializer(
                stddev=0.02))
...
class Decoder(keras.Model):
    ...
    def __init__(self):
        ...
        self.conv1 = keras.layers.Conv2DTranspose(
        ...
        self.conv3 = keras.layers.Conv2D(
            3, kernel_size=7, strides=1, padding='same',
            kernel_initializer=tf.random_normal_initializer(stddev=0.02))
...
def discriminator_loss(disc_of_real_output, disc_of_gen_output, lsgan=True):
    if lsgan:  # Use least squares loss
        ...
        real_loss = keras.losses.mean_squared_error(
            disc_of_real_output, tf.ones_like(disc_of_real_output))
        generated_loss = tf.reduce_mean(tf.square(disc_of_gen_output))

        # 0.5 slows down rate that D learns compared to G
        total_disc_loss = (real_loss + generated_loss) * 0.5
    else:  # Use vanilla GAN loss
        raise NotImplementedError
        real_loss = tf.losses.sigmoid_cross_entropy(
            multi_class_labels=tf.ones_like(disc_of_real_output),
            logits=disc_of_real_output)
        generated_loss = tf.losses.sigmoid_cross_entropy(
            multi_class_labels=tf.zeros_like(disc_of_gen_output),
...
def generator_loss(disc_of_gen_output, lsgan=True):
    if lsgan:  # Use least squares loss
        ...
        gen_loss = keras.losses.mean_squared_error(
            disc_of_gen_output, tf.ones_like(disc_of_gen_output))
    else:  # Use vanilla GAN loss
        raise NotImplementedError
...
def cycle_consistency_loss(
        data_A, data_B, reconstructed_data_A, reconstructed_data_B, cyc_lambda=10):
    loss = tf.reduce_mean(
        tf.abs(data_A - reconstructed_data_A) + tf.abs(data_B - reconstructed_data_B))
